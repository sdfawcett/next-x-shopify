import Image from 'next/image'
import Marquee from "react-fast-marquee";
import RollingStoneLogo from '../images/Rolling-Stone-Logo-cropped.svg'
import MaximLogo from '../images/Maxim-Logo-cropped.svg'
import TimeLogo from '../images/Time-Logo-cropped.svg'
import VarietyLogo from '../images/Variety-Logo-cropped.svg'
import ViceLogo from '../images/Vice-Logo.svg'
import NewYorkerLogo from '../images/The-New-Yorker-Logo.svg'
import EntertainmentWeeklyLogo from '../images/Entertainment-Weekly-Logo.svg'
import NewYorkTimesLogo from '../images/New-York-Times_Logo.svg'

export default function Companies() {
  return (
   <section className='relative marquee border-t-4 border-b-4 border-gray-900 dark:border-new-beige text-gray-900 dark:text-new-beige'>
      <div className="relative flex overflow-x-hidden font-karla">
         <div className="pt-8 pb-5 whitespace-nowrap">

            <Marquee gradient={false} speed={40} loop={0}>
               <span className="mx-16 text-xl text-center">
                  "World's finest CBD brand for relaxation."<br />
                  <Image src={RollingStoneLogo} height={40} width={100} alt='rolling stone logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "The hottest CBD brand around right now."<br />
                  <Image src={MaximLogo} height={40} width={100} alt='maxim logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "Thanks to Bomb Botanicals, we're living our best lives."<br />
                  <Image src={TimeLogo} height={40} width={100} alt='time logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "One of the best THC-free CBD oils that we've ever tried."<br />
                  <Image src={VarietyLogo} height={40} width={100} alt='variety logo' />
               </span>

               <span className="mx-16 text-xl text-center">
                  "The CBD brand at the top of our list for a number of reasons."<br />
                  <Image src={ViceLogo} height={40} width={100} alt='vice logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "FAB CBD continues to push the envelope when it comes to product design and development."<br />
                  <Image src={NewYorkerLogo} height={40} width={100} alt='new yorker logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "In comparison to other CBD brands, FAB CBD’s prices are comparable or more affordable."<br />
                  <Image src={EntertainmentWeeklyLogo} height={40} width={100} alt='entertainment weekly logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "Affordable prices for strong doses."<br />
                  <Image src={NewYorkTimesLogo} height={40} width={100} alt='new york times logo' />
               </span>

               <span className="mx-16 text-xl text-center">
                  "World's finest CBD brand for relaxation."<br />
                  <Image src={RollingStoneLogo} height={40} width={100} alt='rolling stone logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "The hottest CBD brand around right now."<br />
                  <Image src={MaximLogo} height={40} width={100} alt='maxim logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "Thanks to Bomb Botanicals, we're living our best lives."<br />
                  <Image src={TimeLogo} height={40} width={100} alt='time logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "One of the best THC-free CBD oils that we've ever tried."<br />
                  <Image src={VarietyLogo} height={40} width={100} alt='variety logo' />
               </span>

               <span className="mx-16 text-xl text-center">
                  "The CBD brand at the top of our list for a number of reasons."<br />
                  <Image src={ViceLogo} height={40} width={100} alt='vice logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "FAB CBD continues to push the envelope when it comes to product design and development."<br />
                  <Image src={NewYorkerLogo} height={40} width={100} alt='new yorker logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "In comparison to other CBD brands, FAB CBD’s prices are comparable or more affordable."<br />
                  <Image src={EntertainmentWeeklyLogo} height={40} width={100} alt='entertainment weekly logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "Affordable prices for strong doses."<br />
                  <Image src={NewYorkTimesLogo} height={40} width={100} alt='new york times logo' />
               </span>

               <span className="mx-16 text-xl text-center">
                  "World's finest CBD brand for relaxation."<br />
                  <Image src={RollingStoneLogo} height={40} width={100} alt='rolling stone logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "The hottest CBD brand around right now."<br />
                  <Image src={MaximLogo} height={40} width={100} alt='maxim logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "Thanks to Bomb Botanicals, we're living our best lives."<br />
                  <Image src={TimeLogo} height={40} width={100} alt='time logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "One of the best THC-free CBD oils that we've ever tried."<br />
                  <Image src={VarietyLogo} height={40} width={100} alt='variety logo' />
               </span>

               <span className="mx-16 text-xl text-center">
                  "The CBD brand at the top of our list for a number of reasons."<br />
                  <Image src={ViceLogo} height={40} width={100} alt='vice logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "FAB CBD continues to push the envelope when it comes to product design and development."<br />
                  <Image src={NewYorkerLogo} height={40} width={100} alt='new yorker logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "In comparison to other CBD brands, FAB CBD’s prices are comparable or more affordable."<br />
                  <Image src={EntertainmentWeeklyLogo} height={40} width={100} alt='entertainment weekly logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "Affordable prices for strong doses."<br />
                  <Image src={NewYorkTimesLogo} height={40} width={100} alt='new york times logo' />
               </span>

               <span className="mx-16 text-xl text-center">
                  "World's finest CBD brand for relaxation."<br />
                  <Image src={RollingStoneLogo} height={40} width={100} alt='rolling stone logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "The hottest CBD brand around right now."<br />
                  <Image src={MaximLogo} height={40} width={100} alt='maxim logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "Thanks to Bomb Botanicals, we're living our best lives."<br />
                  <Image src={TimeLogo} height={40} width={100} alt='time logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "One of the best THC-free CBD oils that we've ever tried."<br />
                  <Image src={VarietyLogo} height={40} width={100} alt='variety logo' />
               </span>

               <span className="mx-16 text-xl text-center">
                  "The CBD brand at the top of our list for a number of reasons."<br />
                  <Image src={ViceLogo} height={40} width={100} alt='vice logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "FAB CBD continues to push the envelope when it comes to product design and development."<br />
                  <Image src={NewYorkerLogo} height={40} width={100} alt='new yorker logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "In comparison to other CBD brands, FAB CBD’s prices are comparable or more affordable."<br />
                  <Image src={EntertainmentWeeklyLogo} height={40} width={100} alt='entertainment weekly logo' />
               </span>
               <span className="mx-16 text-xl text-center">
                  "Affordable prices for strong doses."<br />
                  <Image src={NewYorkTimesLogo} height={40} width={100} alt='new york times logo' />
               </span>
               
            </Marquee>

         </div>
      </div>
   </section>

  )
}
