        ...
        <div className="pb-12 md:pb-20">
          ...
          <div className="relative">
            ...
            {items.map((item, index) => (
               <div style={item.style} width={item.size} height={item.size}>
                  <Image key={index} className="absolute rounded-full z-10 animate-float" src={item.image} alt={`User ${index + 1}`} onMouseEnter={() => { setActive(index); setCommentOn(true); }} onMouseLeave={() => setCommentOn(false)} />
               </div>
            ))}
